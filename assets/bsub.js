        ...
        */
        ...
        _renderPrices: function (widget) {
            ...
            var variantId = this._getVariantId(widget);
                console.log(planRadioEls.length);
            if (variantId){
                planRadioEls.forEach(function (element) {
                    ...
                    console.log(variantId, sellingPlanId);
        ...
        _renderGroupDiscountSummary: function (widget) {
            ...
            groupRadioEls.forEach(function (element) {
                ...
                var summaryEl = element.querySelector(this.selectors.groupDiscountSummary);
              	var pSymbol = theme.moneyFormat.split('{')[0];
				var pPrice = document.getElementById("meta_price_list_info").value;
                ...
                switch (maxDiscount.type) {
                    case 'fixed_amount':
                    	...
                    	dAmount = pPrice-maxDiscount.value;
                        break;
                    case 'percentage':
                    default:
                        summaryString += ` ${maxDiscount.value}%`;
                    	dAmount = (pPrice*(100-maxDiscount.value))/100;
                }
                

                summaryEl.innerHTML = pSymbol+dAmount+'<span class="offer_amount">'+summaryString+"</span>";
            }.bind(this));
        },
        ...
        _getActiveSellingPlanId: function (widget, groupId) {
            var activePlanInputEl = widget.querySelector(
                `[name=bsub-selling-plan-${groupId}]`,
            )

            if (!activePlanInputEl) {
                console.error(`BSUB: Could not find active plan ID for group ${groupId}.`);
            }
          if(document.getElementById("group_options")){
            return document.getElementById("group_options").value;
          }else{
